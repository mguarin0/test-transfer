...
def _get_surface(model, full_path_to_msms_exec, MSMS="msms"):
    ...
    msms_output_tmp = tempfile.mktemp()
    with open(msms_output_tmp, "w") as file:
        popen = subprocess.Popen([MSMS,
                                  "-probe_radius",
                                  "1.5",
                                  "-if",
                                  xyz_tmp,
                                 ...
                                 stdout=file,
                                 #shell=True,
                                 executable=full_path_to_msms_exec).wait()
    surface_file = surface_tmp + ".vert"
    assert(os.path.isfile(surface_file)), "Failed to generate surface file with msms cmd"

    # read surface vertices from vertex file
    surface = _read_vertex_array(surface_file)

    remove_from_fs(surface_tmp+".vert")
    remove_from_fs(surface_tmp+".face")
    remove_from_fs(msms_output_tmp)
    remove_from_fs(xyz_tmp)

    return surface
# end

def _min_dist(coord, surface):
...
def _ca_depth(residue, surface):
    if not residue.has_id("CA"):
        return None
    ca = residue["CA"]
    coord = ca.get_coord()
    return _min_dist(coord, surface)
# end

def _residue_depth(residue, surface):
    ...
    for atom in atom_list:
        ...
        d = d + _min_dist(coord, surface)
    return d / length
# end

#def residue_depth_features_main(full_path_to_ent_file, full_path_to_msms_exec):
def residue_depth_features_main(kwargs):
    """
    desc:
    args:
    returns:
    ...
    start_time = timer()
    try:
        assert(all(k in ["run_type", "msms_path", "ent_filename"]
                        for k in kwargs.keys())), "incorrect list of args"
        assert(kwargs["run_type"]=="r_residueDepth"), "run type does not match argument passed"
        assert(os.path.exists(kwargs["msms_path"])), "msms path does not exist"
        ...
        all_residue_depth_avg_dst_of_atoms = {}
        all_residue_dst_alpha_carbon = {}
        for model in structure:
            for chain in model:
                chain_id = "{2}_{0}".format(*list(map(str, chain.full_id)))
                surface = _get_surface(model=model, full_path_to_msms_exec=kwargs["msms_path"])
                for i_res, residue in enumerate(chain.get_residues()):
                    ...
                    all_residue_dst_alpha_carbon[(chain_id, res_id)] = _ca_depth(residue, surface)
                    all_residue_depth_avg_dst_of_atoms[(chain_id, res_id)] = _residue_depth(residue, surface)
        to_pickle(all_residue_dst_alpha_carbon,
                  path=os.path.join(paths.RUN_TYPE_OUT_DIR,
                                    "r_residueDepth_ac_{}.p".format(complex_code)))
        to_pickle(all_residue_depth_avg_dst_of_atoms,
                  path=os.path.join(paths.RUN_TYPE_OUT_DIR,
                                    "r_residueDepth_avg_dst_{}.p".format(complex_code)))
        end_time = timer()
        run_time = end_time-start_time
        logging.info("|| complex_code: {} "
                     "|| all_residue_dst_alpha_carbon shape: {} "
                     "|| all_residue_depth_avg_dst_of_atoms shape: {}".format(complex_code,
                                                                              len(all_residue_dst_alpha_carbon),
                                                                              len(all_residue_depth_avg_dst_of_atoms)))
        logging.info("|| complex_code: {} "
