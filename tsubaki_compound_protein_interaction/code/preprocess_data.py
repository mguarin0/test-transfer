...
def make_ijbonddict(mol):
    """
      i_jbond_dict gives dict where:
        BegAtomIdx/EndAtomIdx is a key
        and list of tups with (BegAtomIdx/EndAtomIdx, bond type index) as value
    """
    if verbose:
        print("==== make_ijbonddict function ====")
...
def Weisfeiler_Lehman(atom_list, i_jbond_dict, radius):
    ...
    if verbose:
        print("==== Weisfeiler_Lehman function ====")
...
if __name__ == "__main__":
    ...
    for data in data_file:
        ...
        atom_list = make_atomlist(mol) # list of atom indexes for current smile
        ...
        if ('.' not in smiles):
            ...
            if (len(atom_list) == 1):
                ...
                edge = np.array([[-1]], np.int32) # -1 for edge if atom_list is length 1
                adja_list = [[-1]] # -1 for adja_list if atom_list is length 1
            else:
                vertex, edge = Weisfeiler_Lehman(atom_list,
                                                 i_jbond_dict,
                                                 radius)
            ...
            """Vertex_."""
            vertex_ = make_vertex_(adja_list) # vertex matrix corresponding to adjacency matrix
