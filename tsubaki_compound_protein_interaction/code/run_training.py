...
class CompoundProteinInteraction(Chain):
    def __init__(self):
        super(CompoundProteinInteraction, self).__init__(
              embed_vertex=L.EmbedID(n_vertex, dim, ignore_label=-1), # EmbedID implements so called word embeddings 
              embed_edge=L.EmbedID(n_edge, dim, ignore_label=-1),
              W_vertex=L.Linear(dim, dim), # Linear function, or affine transformation.
    ...
    def gnn(self, vertex, edge, adjacency, vertex_):
        ...
        for _ in range(layer_gnn):
            ...
            h_side = F.relu(self.W_vertex(x_vertex_+x_adja)) # equation 4
            ...
            x_edge = F.sigmoid(F.relu(self.W_edge(x_edge)) + h_side) # equation 5 and 6
            x_edge = F.reshape(x_edge, (V, degree, dim))
